...
class TransactionService:
    ...
    def transfer_to_account(self, sender_user: User, receiver_data: TransferForm):
        sender_account = (
            self.db.query(Account).filter(Account.user_id == sender_user.id).first()
        )
        if not sender_account:
            raise HTTPException(
                status_code=404, detail="no account is linked to the user"
            )
        receiver_account = (
            self.db.query(Account)
            .filter(Account.account_number == receiver_data.receiver_account_number)
            .first()
        )
