...
class AdminService:
    ...
    def create_admin(self, data: AdminCreate):
        admin = (
            self.db.query(Admin)
            .filter(Admin.username == data.username or Admin.email == data.email)
            .first()
        )
        if admin:
            raise HTTPException(
                status_code=401, detail="username or password already exist"
            )
        new_admin = Admin(
            email=data.email,
            name=data.name,
    ...
    def dashboard(self):
        ...
        total_account_balance = self.db.query(
