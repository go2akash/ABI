...
@router.post(
    "/auth/register", response_model=AdminShow, status_code=status.HTTP_201_CREATED
)
def register_admin(db: Annotated[Session, Depends(get_db)], admin_data: AdminCreate):
...
def login_admin(db: Annotated[Session, Depends(get_db)], login_data: AdminLogin):
    admin = authenticate_admin(
        db, login_data.password, login_data.username, login_data.email
    )
    if not admin:
        raise HTTPException(
    ...
    return AdminToken(access_token=access_token, token_type="bearer", admin_id=admin.id)
...
def deposit_to_account(
    db: Annotated[Session, Depends(get_db)],
    current_admin: Annotated[Admin, Depends(get_current_admin)],
    depositor_details: DepositForm,
):
    service = AdminService(db)
    return service.deopsit_to_account(
        depositor_details.receiver_account_number, depositor_details.amount
    )
